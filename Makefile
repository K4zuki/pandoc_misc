...
$(TARGETDIR)/$(TEXTEMPLATE_EXTRA): $(MISC)/$(TEXTEMPLATE_EXTRA)
	$(PANDOC) -f markdown -t latex -M short-hash=$(HASH) --template $< $(MISC)/$(CONFIG) $(MDDIR)/$(CONFIG) -o $@
$(TARGETDIR)/$(TEXTEMPLATE_COVER): $(MISC)/$(TEXTEMPLATE_COVER)
	$(PANDOC) -f markdown -t latex -M short-hash=$(HASH) --template $< $(MISC)/$(CONFIG) $(MDDIR)/$(CONFIG) -o $@
$(TARGETDIR)/$(TEXTEMPLATE_TAIL): $(MISC)/$(TEXTEMPLATE_TAIL)
	$(PANDOC) -f markdown -t latex -M short-hash=$(HASH) --template $< $(MISC)/$(CONFIG) $(MDDIR)/$(CONFIG) -o $@
$(TARGETDIR)/$(TARGET).tex: $(FILTERED) $(TARGETDIR)/$(TEXTEMPLATE_EXTRA) $(TARGETDIR)/$(TEXTEMPLATE_COVER) $(TARGETDIR)/$(TEXTEMPLATE_TAIL)
	$(PANDOC) $(PANFLAGS) $(TEXFLAGS) $(TEXFONTFLAGS) \
		$(FILTERED) -o $(TARGETDIR)/$(TARGET).tex

filtered: $(FILTERED)
$(FILTERED): $(MDDIR)/$(INPUT) $(MARKDOWN) $(CONFIG)
	$(GPP) $(GPPFLAGS) $< > $@
